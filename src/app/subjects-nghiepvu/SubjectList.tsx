...
import { SubjectFormNghiepvu } from "./components/SubjectForm";
...
export function SubjectListNghiepvu() {
  ...
  const loadSubjects = () => {
    startTransition(async () => {
      ...
      // Filter: chỉ hiện subjects có is_criminal=1 HOẶC is_economic=1, KHÔNG phải is_drug=1
      const nghiepvuSubjects = data.filter(s => (s.is_criminal === 1 || s.is_economic === 1) && s.is_drug !== 1);
      setSubjects(nghiepvuSubjects);
      setLoading(false);
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Đối tượng quản lý nghiệp vụ
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
            Danh sách đối tượng hình sự, kinh tế trên địa bàn
          </p>
        </div>
        {canCreate && (
          <Button
      ...
      <div className="flex flex-col gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div className="flex overflow-x-auto pb-1 gap-2 custom-scrollbar">
          <span className="px-4 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-xl text-xs font-bold whitespace-nowrap">
            📋 Tất cả nghiệp vụ
          </span>
          {canApprove && (
            <button
              className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition-all border flex items-center gap-2 ${
                false ? "bg-yellow-500 text-white border-yellow-500 shadow-md" : "bg-yellow-50 text-yellow-600 border-yellow-200 hover:bg-yellow-100"
              }`}
            >
            ...
            {loading || isPending ? (
              ...
              <tr>
                <td colSpan={10} className="text-center py-20 text-slate-400 italic">
                  Không tìm thấy đối tượng nghiệp vụ nào.
                </td>
              </tr>
            ) : (
                ...
                <tr key={s._id?.toString()} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td>
                    <div className="flex flex-wrap gap-1">
                      ...
                      {s.approval_status === "Pending" && <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 text-[10px] font-bold rounded">Chờ duyệt</span>}
                    </div>
                  </td>
                  <td>
      ...
      <div className="md:hidden space-y-4">
        {loading || isPending ? (
          ...
          <div className="text-center py-10 text-slate-400 italic">Không tìm thấy đối tượng nghiệp vụ nào.</div>
        ) : (
            ...
            <div key={s._id?.toString()} className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden group active:scale-[0.98] transition-transform">
               ...
               <div className="mt-4 flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700/50">
                  <div className="flex gap-1">
                    ...
                    {s.is_economic === 1 && <span className="px-2 py-1 bg-emerald-600 text-white text-[9px] font-black rounded-lg shadow-sm">KINH TẾ</span>}
      ...
      <Modal
        ...
        title={editingSubject ? "Chỉnh sửa Đối tượng" : "Thêm Đối tượng Nghiệp vụ"}
        size="xl"
      >
        <SubjectFormNghiepvu
      ...
      >
        {viewingSubject && (
          <SubjectDetailNghiepvu subject={viewingSubject} />
        )}
      </Modal>
    </div>
  );
...
function SubjectDetailNghiepvu({ subject }: { subject: ISubject }) {
  return (
    <div className="text-sm">
      ...
      {/* Classification - Nghiệp vụ */}
      <div className="mb-6">
        <h4 className="text-base font-bold text-slate-800 dark:text-white mb-3 pb-2 border-b-2 border-slate-200 dark:border-slate-700 uppercase tracking-wide">
          Phân loại đối tượng nghiệp vụ
        </h4>
